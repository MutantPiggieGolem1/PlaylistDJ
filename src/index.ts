...
const WHITELIST: Array<string> = ["547624574070816799"] // Me only
...
client.on("messageCreate", (msg: Message) => {
    if (msg.author.id === client.user?.id || !WHITELIST.includes(msg.author.id)) return; // Restrict usage to me only
...
client.on("interactionCreate", async (interaction: Interaction) => {
    if ((!interaction.isCommand() && !interaction.isContextMenu()) || !WHITELIST.includes(interaction.user.id)) return; // Restrict usage to me only
...
client.on("interactionCreate", async (interaction: Interaction) => {
    if ((!interaction.isButton() && !interaction.isSelectMenu()) || !WHITELIST.includes(interaction.user.id)) return;
